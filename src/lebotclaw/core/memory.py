...
class MemoryStore:
    ...
    并发安全：连接以 ``check_same_thread=False`` 打开，配合 WAL + busy_timeout，
    且所有访问由 ``self._lock``（可重入锁）串行化，可在 Web/飞书多线程下共享。
    单连接被多线程并发使用时，仅 ``check_same_thread=False`` 不够（会出现
    "Recursive use of cursors"/事务错乱），故读/写均加锁。
    """
